        <div class="pure-g" id="spgsinfo-topmenu">
			...
			<?php
				...
				$cur_btn_name = $current_btn['name'];

				function get_thisPage($name){
					global $cur_btn_name;
					if($name == $cur_btn_name){
						return "thispage";
					}else{
						return "";
					}
...
//[Mnt-007]<<------------------------------------------------------------------------------------------ Edit E n d 25/02/04
					if($btn_suu <= 6){
						...
						echo '<input type="submit" class="pure-button topbutton '.get_thisPage($cfg_button["name"]).func_btncolor($cfg_button["option"]).'" value="'.$cfg_button["name"].'" >';
						echo form_close();
						echo '</div>';
					}else{
						if($cnt == ($max_button -2-1)){
							...
							echo form_open($cfg_button["url"]);
							echo '<input type="submit" class="pure-button topbutton half-button '.get_thisPage($cfg_button["name"]).func_btncolor($cfg_button["option"]).'" value="'.$cfg_button["name"].'" >';
							echo form_close();
...
//[Mnt-004]<<------------------------------------------------------------------------------------------ Edit E n d 25/02/04
				echo form_open($cfg_button["url"]);
				echo '<button type="submit" class="mobile-menu '.get_thisPage($cfg_button["name"]).func_btncolor($cfg_button["option"]).'">'.mb_convert_kana($cfg_button["name"],"k").'</button>';
				echo form_close();
			}
		?>
	    </div>
