		...
		////////////////////////////////////////////////////////////////////////////
		function naiyou_dsp_urikake(sanshou_id, agency, base_url){
			if((agency-0)==0 || agency===void 0){
				alert("エージェンシーコードが設定されていません。");
			}
			if(base_url=='' || base_url===void 0){
			...
			$.ajax( {
				...
				success: function(json) {
					...
					if(sanshou_idx=='1000'){	// 商品コード1000は契約単価or特単を使う
						if ($("#keiyaku"+stoccd_idx).size() > 0){
							var keiyaku_idx = $("#keiyaku"+stoccd_idx).val()-0;
							if(keiyaku_idx==1){
								$('#'+tanka_id).val($('#keitanka'+stoccd_idx).val());
							}else{
								$('#'+tanka_id).val($('#toktanka'+stoccd_idx).val());
							}
						}else{
							var keiyaku_idx = $("#keiyaku").val()-0;
							alert(keiyaku_idx);
							if(keiyaku_idx==1){
								$('#'+tanka_id).val($('#keitanka').val());
							}else{
								$('#'+tanka_id).val($('#toktanka').val());
							}
						}
						//alert('idx='+stoccd_idx);
						//alert('tanka_id='+tanka_id);
						//alert($('#keitanka'+stoccd_idx).val());
						//alert($('#toktanka'+stoccd_idx).val());
					}else{	// 商品コード1000以外はjson参照で単価を拾う
		...
		function naiyou_dsp_shiire(sanshou_id, agency, base_url){
			...
			$.ajax( {
				...
				//	$('#'+bumoncd_id).val(json[0].bumoncd);
					...
					$('#'+name_id).css('color', '#000');
				//	if(sanshou_idx=='1000'){	// 商品コード1000は契約単価or特単を使う
				//		var keiyaku_idx = parseInt($("#keiyaku").val());
				//		if(keiyaku_idx==1){
				//			$('#'+tanka_id).val($('#keitanka').val());
				//		}else{
				//			$('#'+tanka_id).val($('#toktanka').val());
		...
		function naiyou_dsp_allclear(){
			...
			if($('#youbi_tt').css('display')!='none'){
				$('#youbi_tt').css('display','none');
			}
			if($('#fuzai_tt').css('display')!='none'){
				$('#fuzai_tt').css('display','none');
			}
			...
			if($('#cardshu_tt').css('display')!='none'){
				$('#cardshu_tt').css('display','none');
			}
			...
			if($('#nouhinhenko_tt').css('display')!='none'){
				$('#nouhinhenko_tt').css('display','none');
			}
			if($('#den_tt').css('display')!='none'){
				$('#den_tt').css('display','none');
			}
			if($('#svrsetti_tt').css('display')!='none'){
				$('#svrsetti_tt').css('display','none');
			}
			if($('#svrhiki_tt').css('display')!='none'){
				$('#svrhiki_tt').css('display','none');
			}
			if($('#urikbn_tt').css('display')!='none'){
				$('#urikbn_tt').css('display','none');
			}
			if($('#haiso_tt').css('display')!='none'){
				$('#haiso_tt').css('display','none');
			}
			if($('#jikansitei_tt').css('display')!='none'){
				$('#jikansitei_tt').css('display','none');
			}
			if($('#jikansitei_tt').css('display')!='none'){
				$('#jikansitei_tt').css('display','none');
			}
			if($('#keisan_tt').css('display')!='none'){
				$('#keisan_tt').css('display','none');
			}
			if($('#tukikankaku_tt').css('display')!='none'){
				$('#tukikankaku_tt').css('display','none');
			}
			if($('#shukankaku_tt').css('display')!='none'){
				$('#shukankaku_tt').css('display','none');
			}
			if($('#teikiyoubi_tt').css('display')!='none'){
				$('#teikiyoubi_tt').css('display','none');
			}
			if($('#keimark_tt').css('display')!='none'){
				$('#keimark_tt').css('display','none');
			}
			if($('#nouhinazukari_tt').css('display')!='none'){
				$('#nouhinazukari_tt').css('display','none');
			}
		...
		//	json参照による指定項目の内容表示
		////////////////////////////////////////////////////////////////////////////
		function ivw_naiyou_dsp(target, sanshou_id, dsp_id, base_url){
			// target = 対象とする項目の種類（コントローラのsearch_json参照
			// sanshou_id = 参照する入力欄のid
			// dsp_id = 表示先タグのid（spanタグ）
			//alert(agency);
			...
			// search_json/対象項目/エージェンシーコード/該当コード
			// 下の記述ではエージェンシーコードをformから参照するので、これも引数で持ってくる仕様に変えた方が良いかも
			var jsonUrl = base_url+'ivw/search_json/' + target + '/' + sanshou_idx;
			// $.ajaxでリクエスト（通信結果をキャッシュさせない必要があるため
			// 表示枠用class
			if($('#'+dsp_id).hasClass('naiyou_label')==false){
				$('#'+dsp_id).addClass('naiyou_label');
